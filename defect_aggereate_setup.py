...
def node_gen(A1,A2,A3,B1,B2,B3,n2=72,defects=[],alternating = False):
	...
	for defective_qubits in tom:
		...
		ignore = 0
		if alternating:
			dead_data_loc_z = [dead_data_loc[0]]
			dead_data_loc_x = [dead_data_loc[1]]
		else:
			dead_data_loc_z = dead_data_loc.copy()
			dead_data_loc_x = dead_data_loc.copy()
		for i in range(n2):
			check_name = ('Xcheck',i)
			# left data qubits
			if i not in check_defect_x_locations:
				...
				if len(set(all_conn_x).intersection(dead_data_loc_x)):
		...
		for i in range(n2):
			...
			if len(set(all_conn_z).intersection(dead_data_loc_z)):
				flags.append(check_name)
				#print("Check affected ",check_name)
...
def circuit_gen(args,n=288,num_cycles=12,symmetry = False,flip = False,alternating = False):
	...
	if len(Xbad)!=0:
		...
		if alternating:
			...
			markedx = marked.copy()	
		if symmetry or flipped or alternating:
			for k in range(len(markedx)):
				XA = Xbad[3*k+0]
				XB = Xbad[3*k+1]
				XC = Xbad[3*k+2]
				print("Xfix syndromes are ",XA,XB,XC)
				#Sweep direction from 0 to 5
				#XAB 
				control = XA
				#target = ZB
					...
					for t in range(6):
						...
						#U[lin_order[target],:] = (U[lin_order[target],:] + U[lin_order[control],:]) % 2
							...
							if control in Xtake2:
								Xtake2[control].append(target)
							else:
								Xtake2[control] = [target]
							cycle.append(('CNOT',control,target))
							chk_X[control] +=1
							data_X[target] +=1
							U[lin_order[target],:] = (U[lin_order[target],:] + U[lin_order[control],:]) % 2
...
if __name__ == "__main__":
	...
	for qbits in qubitdamage:
		...
		if repair == "sym":
			sym = True
		bikeCode_damaged = damage_qubit(bikeCode,turnOffQubits=damageQubits,symmetry=sym,flip=fliper,alterning=alternate)
		...
		if sym:
			...
			if multi_type:
				title = './TMP/multi_asymmetric_mydata_final_' + str(n) + '_' + str(k) + '_p_' + str(error_rate) + '_cycles_' + str(num_cycles)+str(situation)+str(style)
			else:	
