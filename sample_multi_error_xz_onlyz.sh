...
set_terminal_title "Sampling OnlyX"
...
while (( $(echo "$current <= $end" | bc -l) )); do
  ...
  python defect_aggereate_setup.py -err $current -repair asym -arg 0 -sit onlyz
  current=$(echo "$current + $increment" | bc -l)
done
python circuit_distance.py -sit onlyz -style zx