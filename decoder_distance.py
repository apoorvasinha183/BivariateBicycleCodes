...
#title = './TMP/multi_asymmetric_mydata_final_' + str(n) + '_' + str(k) + '_p_' + str(error_rate) + '_cycles_' + str(num_cycles)+str(situation)
def decode_file(title,n=12,k=12,d=50,num_trials=10000,num_cycles=1):
    #n = 144
    #k = 12
    #d = 12
    #num_trials = 100000
    ...
    good_trials=0
    bad_trials=0
    pbar = tqdm(range(num_trials),ncols=0)
    for trial in pbar:
        ##print("Here")
        ...
        # Save time 
        pbar.set_description(f'error_rate:{error_rate};errors:{bad_trials};trials:{trial}')
        if bad_trials >= 10000:
            break	
        if 0 :
            x=1
